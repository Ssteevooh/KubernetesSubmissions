...
async function createTable() {
  ...
  await pool.query("INSERT INTO pingpong (counter) SELECT 0 WHERE NOT EXISTS (SELECT * FROM pingpong)")
}
...
async function increaseCounter() {
  const result = await pool.query("UPDATE pingpong SET counter = counter + 1 RETURNING counter")
  return result.rows[0].counter
}
